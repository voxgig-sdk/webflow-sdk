...
describe("SiteDirect", function()
  it("should direct-list-site", function()
    local setup = site_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-site", setup.live and "live" or "unit")
  ...
  it("should direct-load-site", function()
    local setup = site_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-site", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function site_direct_setup(mockres)
