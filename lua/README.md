...
The Lua SDK for the Webflow API — an entity-oriented client using Lua conventions.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/webflow-sdk/releases)),
...
```lua
local item, err = client:Item():load({ collection_id = "example_collection_id", id = "example_id" })
if err then error(err) end
print(item)
```
...
```lua
local item, err = client:Item():load({ id = "item_id", collection_id = "collection_id" })
```
...
```
lua/
├── webflow_sdk.lua    -- Main SDK module
...
The main module (`webflow_sdk`) exports the SDK constructor
