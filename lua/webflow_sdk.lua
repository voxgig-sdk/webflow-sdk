...
WebflowSDK._make_feature = _make_feature


function WebflowSDK.new(options)
  local self = setmetatable({}, WebflowSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WebflowSDK:options_map()
...
function WebflowSDK:get_utility()
  return Utility.copy(self._utility)
end


function WebflowSDK:get_root_ctx()
  return self._rootctx
end


function WebflowSDK:prepare(fetchargs)
...
function WebflowSDK:direct(fetchargs)
...
-- by passing it.
function WebflowSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WebflowSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WebflowSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function WebflowSDK:Collection(data)
...
function WebflowSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WebflowSDK.new(sdkopts)
  sdk.mode = "test"
