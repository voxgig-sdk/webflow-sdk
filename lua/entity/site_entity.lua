...
local SiteEntity = {}
SiteEntity.__index = SiteEntity


function SiteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SiteEntity)
  self._name = "site"
  self._client = client
...
function SiteEntity:get_name()
  return self._name
end


function SiteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SiteEntity.new(self._client, opts)
end
...
function SiteEntity:mark_deleted()
  self._deleted = true
end


function SiteEntity:deleted()
  return true == self._deleted
end


function SiteEntity:data_set(args)
  if args ~= nil then
...
function SiteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SiteEntity:match_set(args)
  if args ~= nil then
...
function SiteEntity:stream(action, args, callopts)
...
---@param reqmatch SiteLoadMatch
---@param ctrl? table
---@return Site
---@return string? err
function SiteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch SiteListMatch
---@param ctrl? table
---@return Site[]
---@return string? err
function SiteEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function SiteEntity:_run_op(ctx, post_done)
  local utility = self._utility
