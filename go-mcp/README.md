...
[MCP](https://modelcontextprotocol.io) server exposing the Webflow SDK as
two agent tools — `webflow_list` and `webflow_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/webflow-mcp)
make build

# 2. Provide credentials via the environment
export WEBFLOW_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user webflow \
  -- /absolute/path/to/webflow-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./webflow-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// webflow_list: first page of records
{ "entity": "collection" }
{ "entity": "collection", "query": { } }

// webflow_load: one record by id
{ "entity": "collection", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/webflow-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user webflow \
     -- "$PWD"/dist/*/webflow-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `webflow_list` and `webflow_load` tools now appear
   in new sessions. Ask the agent to *"list collection using webflow"*
   and it calls `webflow_list` with `{"entity":"collection"}`.
...
```sh
export WEBFLOW_APIKEY=sk_live_xxx            # API key
export WEBFLOW_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `webflow_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `webflow_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WEBFLOW_APIKEY` | API key sent with every request. |
| `WEBFLOW_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

collection | item | site

### Smoke test via HTTP (raw JSON-RPC)

```sh
./webflow-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"webflow_load","arguments":{"entity":"collection","query":{"id":1}}}}'
```
