...
The Python SDK for the Webflow API — an entity-oriented client following Pythonic conventions.
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/webflow-sdk/releases)) or
...
Item is nested under collection, so provide the `collection_id`.
`load()` returns the ENTITY — call data_get() for the record — and raises on error.

```python
try:
    item = client.Item().load({"collection_id": "example_collection_id", "id": "example_id"})
    print(item)
except Exception as err:
...
```python
client = WebflowSDK.test(testopts, sdkopts)
```
...
```python
item = client.Item().load({"id": "item_id", "collection_id": "collection_id"})
```

#### Example: List

```python
items = client.Item().list({"collection_id": "example"})
```
...
```
py/
├── webflow_sdk.py         -- Main SDK module
...
The main module (`webflow_sdk`) exports the SDK class.
