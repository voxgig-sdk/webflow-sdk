...
class SiteEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new SiteEntity($this->_client, $opts);
    }

    /**
     * @param Site|array $args Site data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = WebflowHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Site|array The current Site data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Site fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = WebflowHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Site fields).
     */
     ...
     * Load a single Site.
     *
     * @param SiteLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed SiteLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Site|array The loaded Site as an assoc-array at the
     *   SDK boundary; throws WebflowError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = WebflowHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param SiteListMatch|array|null $reqmatch Match filter (any subset
     *   of Site fields) as an assoc-array; SiteListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Site[]|array A list of Site items as assoc-arrays at
     *   the SDK boundary; throws WebflowError on failure (item-5 convention).
     */
