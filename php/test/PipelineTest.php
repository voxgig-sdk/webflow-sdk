...
{
    private static function utility(): WebflowUtility
    {
        return new WebflowUtility();
    }
    ...
    private static function ctx(array $over = []): WebflowContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new WebflowContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new WebflowOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof WebflowError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new WebflowOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = WebflowMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new WebflowOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = WebflowMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new WebflowOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = WebflowMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WebflowSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = WebflowMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new WebflowResponse([]),
            'result' => new WebflowResult([]),
        ]);
        [, $err] = WebflowMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new WebflowSpec([]),
            'response' => null,
            'result' => new WebflowResult([]),
        ]);
        [, $err] = WebflowMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new WebflowSpec(['step' => 's']),
            'response' => new WebflowResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new WebflowResult([]),
        ]);
        [, $err] = WebflowMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new WebflowSpec(['step' => 's']),
            'response' => new WebflowResponse(self::resp(200, ['v' => 1])),
            'result' => new WebflowResult([]),
        ]);
        [, $err] = WebflowMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new WebflowSpec(['step' => 's']),
            'response' => new WebflowResponse(self::resp(200, ['v' => 2])),
            'result' => new WebflowResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        WebflowMakeResponse::call($ctx);
    ...
    {
        $preset = new WebflowResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new WebflowSpec([]),
            'response' => new WebflowResponse([]),
            'result' => new WebflowResult([]),
        ...
        [$got, $err] = WebflowMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new WebflowResult([])]);
        [, $err] = WebflowMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new WebflowSpec([]), 'result' => null]);
        [, $err] = WebflowMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WebflowSpec(['step' => 's']),
            'result' => new WebflowResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new WebflowOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WebflowMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WebflowSpec(['step' => 's']),
            'result' => new WebflowResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new WebflowOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WebflowMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WebflowResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new WebflowSpec([]),
            'result' => new WebflowResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = WebflowMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WebflowMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new WebflowError('boom', 'boom');
        $utility->fetcher = function (WebflowContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WebflowSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = WebflowMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (WebflowContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = WebflowMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(WebflowResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (WebflowContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WebflowSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = WebflowMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new WebflowResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new WebflowSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = WebflowMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = WebflowMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = WebflowMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new WebflowResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, WebflowDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new WebflowResult(['ok' => false])]);
        $this->expectException(WebflowError::class);
        WebflowDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new WebflowResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', WebflowMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        WebflowMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new WebflowBaseFeature();
        $b = new WebflowBaseFeature();
        WebflowFeatureAdd::call($ctx, $a);
        WebflowFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): WebflowBaseFeature
    {
        $f = new WebflowBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        WebflowFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): WebflowContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new WebflowSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WebflowPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new WebflowResponse(['headers' => null]),
            'result' => new WebflowResult([]),
        ]);
        WebflowResultHeaders::call($ctx);
