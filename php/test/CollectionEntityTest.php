...
class CollectionEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = WebflowSDK::test($seed, null);
        $seen = iterator_to_array($base->Collection(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = WebflowConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = WebflowSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set WEBFLOW_TEST_COLLECTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function collection_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["collection01", "collection02", "collection03", "site01", "site02", "site03"] as $k) {
    ...
    $entid_env_raw = getenv("WEBFLOW_TEST_COLLECTION_ENTID");
    ...
    if ($env["WEBFLOW_TEST_LIVE"] === "TRUE") {
        ...
        $client = new WebflowSDK(Helpers::to_map($merged_opts) ?? []);
    }
