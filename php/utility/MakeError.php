...
class WebflowMakeError
{
    public static function call(?WebflowContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WebflowContext([], null);
        }
        $op = $ctx->op ?? new WebflowOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WebflowResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WebflowError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new WebflowError('', $msg, $ctx);
