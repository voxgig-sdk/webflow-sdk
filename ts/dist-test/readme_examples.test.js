...
const TEST_SEED = { "entity": { "collection": { "test01": { "id": "test01" } }, "item": { "test01": { "id": "test01" } }, "site": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+WebflowSDK\b/.test(code) ||
        /\bWebflowSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+WebflowSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/WebflowSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
