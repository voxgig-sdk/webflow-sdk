# Webflow TypeScript SDK Reference

Complete API reference for the Webflow TypeScript SDK.
...
#### `WebflowSDK.test(testopts?, sdkopts?)`
...
**Returns:** `WebflowSDK` instance in test mode.
...
```ts
const results = await client.Collection().list({ site_id: "example" })
```
...
Return the parent `WebflowSDK` instance.
...
| `fieldData` | `Record<string, any>` | No |  |
| `id` | `string` | No |  |
...
```ts
const results = await client.Item().list({ collection_id: "example" })
```
...
```ts
const result = await client.Item().load({ id: 'item_id', collection_id: 'collection_id' })
```
