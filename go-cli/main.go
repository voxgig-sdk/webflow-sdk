// Command Webflow-cli is a CLI + REPL for the Webflow SDK,
...
const prompt = "webflow"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "collection item site"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WEBFLOW_APIKEY (api key), WEBFLOW_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WEBFLOW_APIKEY carries the API key and
	// WEBFLOW_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WEBFLOW_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WEBFLOW_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWebflowSDK(opts)
