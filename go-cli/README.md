...
boru-driven command-line client **and** interactive REPL for the Webflow
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/webflow-cli)
make build
...
./webflow-cli list collection
./webflow-cli load 1 collection            # {id:1} shorthand
./webflow-cli load '{id:1}' collection       # explicit match map
./webflow-cli list item

# 5. Override the API base URL for a single call
WEBFLOW_BASE=https://api.example.com ./webflow-cli list collection

# 6. No arguments -> interactive REPL
./webflow-cli
webflow> list collection
webflow> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/webflow-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/webflow-cli list collection
   ```
...
```sh
./webflow-cli load 1 collection          # scalar shorthand for {id:1}
./webflow-cli load '{id:1}' collection     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `webflow>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/webflow-cli`. |
...
(`./webflow-cli <expr>`) and interactively (the REPL), and expressions compose the
