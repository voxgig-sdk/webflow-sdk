...
//
// NewWebflowSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.WebflowSDK {
	res, _ := kase["res"].([]any)
...
func fcCandidates(client *sdk.WebflowSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.WebflowSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.WebflowSDK, name string) any {
