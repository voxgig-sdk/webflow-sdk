...
import (
	...
	omni "github.com/voxgig-sdk/webflow-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WebflowError); is {
				out := map[string]any{
					"name":    "WebflowError",
...
func omniCtx(arg any, client *sdk.WebflowSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WebflowSDK); is {
			client = live
