# Webflow Golang SDK Reference

Complete API reference for the Webflow Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *WebflowSDK`
...
```go
result, err := client.Item(nil).Load(map[string]any{"id": "item_id", "collection_id": "collection_id"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
