...
class FtCtrl extends WebflowControl
{
...
class FtClient
{
    ...
    private WebflowUtility $utility;

    public function __construct(WebflowUtility $utility, array $options)
    {
    ...
    public function get_utility(): WebflowUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WebflowContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new WebflowUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (WebflowContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WebflowConfig::make_feature($name);
    ...
    private static function buildUrl(WebflowSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WebflowError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WebflowResponse($fetched);
        }
    ...
    private function fail(WebflowContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WebflowContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
